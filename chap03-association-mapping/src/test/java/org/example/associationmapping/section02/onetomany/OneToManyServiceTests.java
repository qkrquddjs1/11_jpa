...
public class OneToManyServiceTests {
    ...
    void oneToManyFindTest() {
        //given
        int categoryCode = 4; // 주어진 카테고리 코드
        //when
        Category category = oneToManyService.findCategory(categoryCode); // 주어진 카테고리 코드를 사용하여 카테고리를 조회

        //then
        assertNotNull(category); // 조회된 카테고리가 null이 아닌지 확인
    }
